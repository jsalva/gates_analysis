...
if not len(sys.argv) == 4:
    ...
    sys.exit("Paradigm must be one of the following: 'WM','WMSTAT','Nback_letters','MCNAB'")
...
def timeseries_design(subject_id,whatParadigm,onsets_dir):
    ...
    regressors = None
    onsets_temp = os.path.join(onsets_dir, subject_id+'_*onsets_mcnab.mat')
...
datasink.inputs.base_directory = os.path.join(study_dir, 'l1_mcnab')
...
l1pipeline.base_dir = os.path.abspath('/mindhive/scratch/jsalva/mcnab/%s/'%(paradigm))
l1pipeline.connect([
                    ...
                    (realign,datasink,[('realignment_parameters','qc_realign.@realign_params'),
                                        ('realigned_files','qc_realign.@realigned_files')]),
...
l2datasource.inputs.template = 'l1_mcnab/*/norm_func/con_%04d*'
...
l2datasink = pe.Node(interface=nio.DataSink(), name="l2_mcnab_model1_adults")
l2datasink.inputs.base_directory = os.path.join(study_dir, 'l2_mcnab_model1_adults/')

l2pipeline = pe.Workflow(name='l2_mcnab_filtering_adults')
