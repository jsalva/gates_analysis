...
datasink_dir = '/mindhive/gablab/GATES/Analysis/poster_func_roi_analysis'
...
rois = glob(os.path.join('/mindhive/gablab/GATES',roi_file_template))
for roi in rois:
    name = os.path.split(roi)[1].split('.')[0]
    ROI = pe.Workflow(name="ROI_%s"%(name))
    ...
    ROI.connect([(datasource,unwarp_rois,
        [('seg_file','inputspec.images_to_warp')])])
    ROI.connect([(datasource,unwarp_rois,
        [('brain','inputspec.reference_anatomical'),
        ('inverse_warp_file','inputspec.inverse_warp'),
        ('affine_file','inputspec.affine')])])

    functional_segstats = create_segstats_workflow('fucntional_segstats', regions)
    ...
    ROI.connect([(unwarp_rois,f_segstats_inputspec,
        [(('outputspec.warped_images',getfirst),'segmentation_file')])])
    ...
    ROI.connect([(functional_segstats,datasink,
        [('outputspec.summary_files','functional_%s'%(name))])])
    ...
    statsgrabber.inputs.template ='*/functional_%s*.stats'%(name)
    ...
    STATS.connect([(combine_stat_files,statsink,
        [('combined_summary_file','%s_summary_file'%(name))])])
