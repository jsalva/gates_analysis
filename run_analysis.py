...
os.environ['MPLCONFIGDIR']='/mindhive/scratch/jsalva/'
##############################################################################
#                               ARGUMENTS
##############################################################################
...
def timeseries_design(subject_id,whatParadigm,onsets_dir):
    ...
    regressors = None
    onsets_temp = os.path.join(onsets_dir, subject_id+'_*onsets_durations.mat')
...
datasink.inputs.base_directory = os.path.join(study_dir, 'l1sink_allnodes_durations')
...
l1pipeline = pe.Workflow(name="l1_allnodes_motion_outliers_removed_dilated_durations_modeled")
...
l1pipeline.base_dir = os.path.abspath('/mindhive/gablab/GATES/tmp_working/111312/%s/'%(paradigm))
l1pipeline.connect([
...
l2datasource.inputs.template = 'l1sink_*durations/*/norm_func/con_%04d*'
...
l2datasink = pe.Node(interface=nio.DataSink(), name="l2sink_kids_with_durations")
l2datasink.inputs.base_directory = os.path.join(study_dir, 'l2sink_kids_with_durations/')

l2pipeline = pe.Workflow(name='l2_kids_with_durations')
