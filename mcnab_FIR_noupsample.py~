...
def model_fir_from_onsets(subject_id, modeled_event_name,timeseries_files, onsets_files, motion_param_files, art_outlier_files):
    ...
    for run in range(num_runs):        
        ...
        interp_timeseries = interp1d(np.arange(0,len(timeseries)*repitition_time,repitition_time),timeseries,kind='linear',axis=0,bounds_error=False,fill_value=0)
        ...
        interp_motion_func = interp1d(np.arange(0,len(timeseries)*repitition_time,2),motion_parameters,kind='nearest',axis=0,bounds_error=False,fill_value=0)
